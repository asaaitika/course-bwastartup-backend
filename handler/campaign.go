...
func (h *campaignHandler) GetCampaign(c *gin.Context) {
	...
	if err != nil {
		response := helper.APIResponse("Failed to get all detail of campaign", http.StatusBadRequest, "error", nil)
		c.JSON(http.StatusBadRequest, response)
		return
	}
...
/*1. tangkap parameter dari user ke input struct
2. ambil current user dari jwt/handler
3. panggil service dengan parameter input dari sebuah struct (inputan user), juga membuat slug berdasarkan nama campaign
4. panggil repository utk simpan data campaign baru*/
...
/*1. membuat repository update campaign
2. service mamanggil repository dengan isikan input dari user dan juga input dari uri
3. adanya mapping input (ada 2 inputan) ke struct dalam handler*/
...
/*1. repository: create image/save data image ke dalam table campaign_images;
ubah is_primary true ke false
2. service: perlunya kondisi utk memanggil point 2 dari repo lalu memanggil repo point 1
3. handler: tangkap input lalu mengubah ke struct input lalu, save image campaign di suatu folder*/
